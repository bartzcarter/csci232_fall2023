...
public class BST<K extends Comparable<K>, V>{
    ...
    public void inOrder(ArrayList<Data> list){
        inOrder(root, list);
    }
    //recursive helper method inorder()
    private void inOrder(TreeNode node, ArrayList<Data> list){
        if(node != null){
            inOrder(node.left, list);
            list.add((Data)node.Value);
    ...
    private void preorder(TreeNode node){
        if(node != null){
            System.out.print(node.Key + " " + node.Value.toString() + ", ");
            preorder(node.left);
            preorder(node.right);
        }
    }
