/***
 * Class to model a HashMap that will use linear probing
 * @author Carter Bartz 
 * @version 0.1
...
public class HashMapLP <K, V> {
	...
	public V get(K key) {
		...
		while(hashTable[HTIndex] != null) {
			...
			if(hashTable[HTIndex].getKey().equals(key)){
				return (V)hashTable[HTIndex].getValue();
            }
            HTIndex++;
		}
		return null;
    ...
    public V put(K key, V value) {
	    if (get(key) != null) { // The key is in the hash map
		    int HTIndex = hash(key.hashCode());
            if(hashTable[HTIndex].getKey().equals(key)) {
                V old = (V)hashTable[HTIndex].getValue();
                hashTable[HTIndex].setValue(value); 
                return old;
            }
			else{
				while(!(hashTable[HTIndex].getKey().equals(key))){
					HTIndex++;
				}
				V old = (V)hashTable[HTIndex].getValue();
				hashTable[HTIndex].setValue(value); 
        ...
        while(hashTable[htIndex] != null){
            ...
            else if(htIndex == hashTable.length - 1){
                htIndex = 0;
            }   
        }
